...
def refresh_data():
    from .whoop_api import get_today, get_history
...
async def _apply_prescription(prescription: dict):
    ...
    from .devices import set_fan_speed
...
async def _process_cli_command(line: str, messages: list) -> list:
    ...
    if line in mood_map:
        from .mood import set_mood_override
...
async def main_async():
    ...
    from .devices import connect_fan
    from .config import get

    if not all([get("TAPO_IP") or get("TAPO_DEVICE_IP"), get("TAPO_EMAIL"), get("TAPO_PASSWORD")]):
    ...
    from .whoop_api import get_profile
    profile = get_profile()
...
def main():
    """CLI entry point for soma-scheduler."""
    run()
