...
from .config import FRONTEND_DIR, SCHEDULER_LOG, SOMA_LOG
...
async def startup():
    ...
    from .light import connect_light
...
async def bedtime_signal():
    if not state.get("light"):
        raise HTTPException(status_code=503, detail="Light not connected")
    from .sleep_prep import bedtime_signal as do_bedtime_signal
...
def main():
    """CLI entry point for soma-dashboard."""
    import uvicorn
